...
pdfFiles = []
for filename in os.listdir('.\H07063'): # '.'表示当前目录下 
    if filename.endswith('.pdf'): 
        pdfFiles.append(filename)
pdfFiles.sort(key = str.lower)

directory = 'D:\\03_program\\python\\H07063\\'
...
for filename in pdfFiles:
    pdfFileObj = open(directory+ filename,'rb')  # 将文档以二进制打开赋值给一个File对象： pdfFileObj
...
pdfOutput = open(directory+'all.pdf','wb')  # 将一个pdf文件赋予二进制写入并生成一个文件对象： pdfOutput
